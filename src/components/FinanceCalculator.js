...
const FinanceCalculator = ({ vehiclePrice = 450000000, onAction }) => {
  ...
  return (
    <View style={getStyles(theme).container}>
      ...
      <Text style={[getStyles(theme).subtitle, { color: theme.colors.subtext }]}>
        Giá xe: {formatCurrency(vehiclePrice)}
      </Text>
      ...
      <GlassCard style={getStyles(theme).card} intensity={20}>
        <View style={getStyles(theme).row}>
          ...
          <Text style={getStyles(theme).value}>
            {formatCurrency((vehiclePrice * downPaymentPercent) / 100)}
          </Text>
        </View>
        <Slider
        ...
        >
          <Text style={[getStyles(theme).resultLabel, { color: theme.colors.subtext }]}>
            Ước tính trả mỗi tháng
          </Text>
          <Text style={[getStyles(theme).resultValue, { color: theme.colors.primary }]}>
            {formatCurrency(monthlyInstallment)}
          </Text>
          <Text style={[getStyles(theme).interestHint, { color: theme.colors.success }]}>
            Lãi suất ưu đãi: 8.0%/năm
          </Text>
        </LinearGradient>
        ...
        <View style={getStyles(theme).actionRow}>
          ...
          >
            ...
            <Text style={[getStyles(theme).secondaryBtnText, { color: theme.colors.primary }]}>
              Tư vấn
            </Text>
          </TouchableOpacity>
