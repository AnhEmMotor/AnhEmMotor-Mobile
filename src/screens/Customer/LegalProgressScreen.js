...
export default function LegalProgressScreen() {
  const steps = [
    ...
    {
      id: 3,
      title: 'Đang làm thủ tục đóng thuế',
      date: '05/05/2026',
      completed: true,
      icon: CheckCircle2,
      iconColor: 'success',
    ...
    {
      id: 4,
      title: 'Đợi bấm biển số',
      date: 'Dự kiến: 10/05/2026',
      completed: false,
      icon: Clock,
      iconColor: 'subtext',
    },
    ...
    <ScrollView style={getStyles(theme).container} showsVerticalScrollIndicator={false}>
      <Animated.View entering={FadeInUp.duration(600).delay(100)}>
        ...
        <View style={getStyles(theme).headerInfo}>
          <Text style={[getStyles(theme).bikeName, { color: theme.colors.primary }]}>
            Kawasaki Z1000
          </Text>
          <Text style={[getStyles(theme).bikePlate, { color: theme.colors.subtext }]}>
            Biển số: 59-A3 123.45
          </Text>
        </View>
      </Animated.View>
        ...
        {steps.map((step, index) => (
          ...
          >
            <View style={getStyles(theme).leftLine}>
              <View
                style={[
                  getStyles(theme).iconWrapper,
                  {
                    backgroundColor: theme.colors[step.iconColor] + '20',
                    borderColor: theme.colors.border,
                  },
                ]}
              >
                {React.createElement(step.icon, {
                  color: theme.colors[step.iconColor],
                  size: 20,
                })}
              </View>
            ...
            <View style={getStyles(theme).rightContent}>
              <GlassCard
                style={[
                  ...
                  {
                    borderColor: step.completed ? theme.colors.success + '40' : theme.colors.border,
              ...
              >
                <Text
                  style={[
                    getStyles(theme).stepTitle,
                    {
                      color: step.completed ? theme.colors.text : theme.colors.subtext,
                    },
                  ]}
                >
                  {step.title}
                ...
                <View style={getStyles(theme).dateRow}>
                  ...
                  <Text style={[getStyles(theme).stepDate, { color: theme.colors.subtext }]}>
                    {step.date}
                  </Text>
                </View>
              </GlassCard>
      ...
      <Animated.View entering={FadeInUp.duration(800).delay(1000)}>
        ...
        >
          <Text style={styles.infoText}>
            Hồ sơ của bạn đang được xử lý ưu tiên. Vui lòng chuẩn bị CCCD bản gốc khi đi bấm biển.
          </Text>
        </LinearGradient>
      </Animated.View>
