...
import {
  ChevronLeft, FileCheck2, Building2, Banknote,
  CalendarDays, Car, Shield, MapPin, CheckCircle2,
  AlertCircle, Clock, ChevronRight, Fingerprint, Hash
...
function StatusBadge({ status }) {
  ...
  const Icon = cfg.icon;
  return (
    <View style={[badge.wrap, { backgroundColor: cfg.color + '1A', borderColor: cfg.color + '40' }]}>
...
function InfoRow({ icon: Icon, label, value, valueColor, colors }) {
  ...
  return (
    <View style={[row.container, { borderBottomColor: colors.border }]}>
      ...
      <Text style={[row.value, { color: valueColor ?? colors.text }]} numberOfLines={2}>{value}</Text>
    </View>
  );
}
const row = StyleSheet.create({
  container: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 13, borderBottomWidth: StyleSheet.hairlineWidth },
    ...
    <SafeAreaView style={[s.root, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
      ...
      {loading ? (
        ...
        <View style={s.center}>
          ...
          <Text style={[s.hint, { color: colors.subtext }]}>Bạn chưa có hợp đồng mua xe nào với AnhEmMotor.</Text>
        </View>
      ) : (
        <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={s.list}>
          <Text style={[s.sectionLabel, { color: colors.subtext }]}>{contracts.length} hợp đồng tìm thấy</Text>
          ...
          {contracts.map((c) => (
            ...
            >
              ...
              <View style={[s.cardFooter, { borderTopColor: colors.border }]}>
                <Text style={[s.footerDate, { color: colors.subtext }]}>Ngày ký: {c.signedDate ?? '—'}</Text>
      ...
      <Modal visible={!!selected} transparent animationType="slide" onRequestClose={() => setSelected(null)}>
        <View style={[s.modalOverlay, { backgroundColor: colors.modalOverlay }]}>
          <TouchableOpacity style={StyleSheet.absoluteFillObject} onPress={() => setSelected(null)} activeOpacity={1} />
          <View style={[s.modalSheet, { backgroundColor: colors.card }]}>
            <View style={[s.handle, { backgroundColor: colors.border }]} />
            <Text style={[s.modalTitle, { color: colors.text }]}>Chi tiết hợp đồng</Text>
            <Text style={[s.modalSub, { color: colors.subtext }]}>{selected?.contractNumber} · {selected?.signedDate}</Text>

            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 420 }}>
              <Text style={[s.groupTitle, { color: colors.subtext }]}>🚗 THÔNG TIN XE</Text>
              <View style={[s.group, { borderColor: colors.border }]}>
                <InfoRow icon={Car} label="Dòng xe" value={selected?.vehicleModel} colors={colors} />
                <InfoRow icon={Car} label="Phiên bản" value={selected?.vehicleVersion} colors={colors} />
                <InfoRow icon={Car} label="Màu sắc" value={selected?.vehicleColor} colors={colors} />
                <InfoRow icon={Hash} label="Số khung" value={selected?.frameNumber} colors={colors} />
                <InfoRow icon={Hash} label="Số máy" value={selected?.engineNumber} colors={colors} />
              </View>

              <Text style={[s.groupTitle, { color: colors.subtext, marginTop: 16 }]}>💰 TÀI CHÍNH</Text>
              <View style={[s.group, { borderColor: colors.border }]}>
                <InfoRow icon={Banknote} label="Giá bán thực tế" value={fmt(selected?.actualSalePrice)} valueColor="#A855F7" colors={colors} />
                <InfoRow icon={Banknote} label="Tiền đặt cọc" value={fmt(selected?.depositAmount)} valueColor="#F59E0B" colors={colors} />
                <InfoRow icon={Banknote} label="Còn phải trả" value={fmt(selected?.remainingAmount)} valueColor="#EF4444" colors={colors} />
                <InfoRow icon={CalendarDays} label="Hạn thanh toán" value={selected?.finalPaymentDeadline} colors={colors} />
              </View>

              <Text style={[s.groupTitle, { color: colors.subtext, marginTop: 16 }]}>🛡️ BẢO HÀNH</Text>
              <View style={[s.group, { borderColor: colors.border }]}>
                <InfoRow icon={Shield} label="Thời hạn BH" value={selected?.warrantyPeriod} colors={colors} />
                <InfoRow icon={Shield} label="Phạm vi BH" value={selected?.warrantyScope} colors={colors} />
              </View>

              <Text style={[s.groupTitle, { color: colors.subtext, marginTop: 16 }]}>🏢 ĐẠI LÝ</Text>
              <View style={[s.group, { borderColor: colors.border }]}>
                <InfoRow icon={Building2} label="Đại lý" value={selected?.showroomName} colors={colors} />
                <InfoRow icon={MapPin} label="Địa chỉ" value={selected?.showroomAddress} colors={colors} />
                <InfoRow icon={Fingerprint} label="Đại diện" value={selected?.showroomRepresentative} colors={colors} />
              </View>
            </ScrollView>
...
const s = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 11, marginBottom: 12, fontWeight: '600', textTransform: 'uppercase', letterSpacing: 0.6 },
  ...
  cardFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, borderTopWidth: StyleSheet.hairlineWidth },
