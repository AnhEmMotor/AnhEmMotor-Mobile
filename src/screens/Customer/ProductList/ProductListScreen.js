    ...
    >
      <View style={[styles.header, { borderBottomColor: activeColors.border }]}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text style={[styles.headerTitle, { color: activeColors.text }]}>
            {type !== 'Tất cả' ? type : 'Sản phẩm'}
          </Text>
          <Text style={[styles.headerSubtitle, { color: activeColors.subtext }]}>
            {brand !== 'Tất cả' ? brand : 'Tất cả thương hiệu'}
          </Text>
        </View>
      ...
      >
        <View style={styles.resultsCount}>
          <Text style={[styles.countText, { color: activeColors.subtext }]}>
            ...
            >
              ...
              >
                <Image
                  ...
                  source={
                    item.img || item.imageUrl
                      ? typeof (item.img || item.imageUrl) === 'string'
                        ? { uri: item.img || item.imageUrl }
                        : item.img || item.imageUrl
                      : {
                          uri: 'https://images.unsplash.com/photo-1558981403-c5f9899a28bc?q=80&w=2070',
                        }
                  }
                ...
                <View style={styles.productInfoOpen}>
                  <Text
                    style={[styles.productNameOpen, { color: activeColors.text }]}
                    numberOfLines={2}
                  >
                    {item.name}
                  </Text>
                  <Text style={[styles.productPriceOpen, { color: activeColors.primary }]}>
                    {item.price || item.referencePrice || 'Liên hệ'}
                  </Text>
                </View>
              </ScalePress>
            </Animated.View>
