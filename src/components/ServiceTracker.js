...
const ServiceTracker = ({ currentStep = 2 }) => {
  ...
  return (
    <View style={getStyles(activeColors).container}>
      <Text style={[getStyles(activeColors).sectionTitle, { color: activeColors.text }]}>
        Theo dõi dịch vụ 🛠️
      </Text>
      <GlassCard style={getStyles(activeColors).card} intensity={15}>
              ...
              <View key={step.id} style={getStyles(activeColors).stepItem}>
                  ...
                  {isActive ? (
                    <PulseView pulseScale={1.2}>
                      <View
                        style={[
                          ...
                          getStyles(activeColors).activeCircle,
                    ...
                    <View
                      style={[
                        getStyles(activeColors).iconCircle,
                        isCompleted
                          ? getStyles(activeColors).completedCircle
                          : [
                              getStyles(activeColors).pendingCircle,
                              {
                  ...
                  {index < steps.length - 1 && (
                    <View
                      style={[
                        ...
                        isCompleted
                          ? [
                              getStyles(activeColors).completedConnector,
                              { backgroundColor: activeColors.success },
                            ]
                          : [
                              getStyles(activeColors).pendingConnector,
                              {
                                backgroundColor: activeColors.isDark
                                  ? 'rgba(255,255,255,0.05)'
                                  : 'rgba(0,0,0,0.04)',
                ...
                <Text
                  style={[
                    ...
                    isActive
                      ? [getStyles(activeColors).activeText, { color: activeColors.primary }]
                      : [getStyles(activeColors).pendingText, { color: activeColors.subtext }],
                  ]}
                >
                  {step.title}
                </Text>
        ...
        >
          <Text style={[getStyles(activeColors).infoLabel, { color: activeColors.subtext }]}>
            Trạng thái hiện tại:
          </Text>
          <Text style={[getStyles(activeColors).infoValue, { color: activeColors.text }]}>
            {steps.find((s) => s.id === currentStep)?.title} - Kỹ thuật viên đang kiểm tra tổng
            quát.
          </Text>
        </View>
      </GlassCard>
