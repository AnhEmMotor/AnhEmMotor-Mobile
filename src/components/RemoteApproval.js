    ...
    <View style={getStyles(activeColors).container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
        {items.map((item) => (
          <GlassCard
            ...
            intensity={item.status === 'rejected' ? 5 : 15}
          >
            <View style={styles.itemInfo}>
              <View style={styles.itemNameRow}>
                <Text
                  style={[
                    getStyles(activeColors).itemName,
                    { color: activeColors.text },
                    item.status === 'rejected' && [
                      getStyles(activeColors).rejectedText,
                ...
                {item.urgent && (
                  <View style={getStyles(activeColors).urgentBadge}>
                    <Text
                      style={[
                        getStyles(activeColors).urgentText,
            ...
            <View
              style={[
                styles.actionRow,
                {
                  borderTopColor: activeColors.isDark
            ...
            >
              <TouchableOpacity
                style={[
                  getStyles(activeColors).actionBtn,
                  item.status === 'rejected' && [
                    getStyles(activeColors).activeReject,
              ...
              >
                ...
                <Text
                  style={[
                    getStyles(activeColors).actionText,
                    { color: activeColors.subtext },
                    item.status === 'rejected' && getStyles(activeColors).activeLabel,
                  ]}
                >
                  Từ chối
                </Text>
              ...
              <TouchableOpacity
                style={[
                  ...
                  item.status === 'approved' && [
                    getStyles(activeColors).activeApprove,
              ...
              >
                <Check
                  color={item.status === 'approved' ? '#fff' : activeColors.subtext}
                  size={18}
                />
                <Text
                  style={[
                    getStyles(activeColors).actionText,
                    { color: activeColors.subtext },
                    item.status === 'approved' && getStyles(activeColors).activeLabel,
                  ]}
                >
                  Đồng ý
                </Text>
      ...
      <View
        style={[
          styles.footer,
          {
            borderTopColor: activeColors.isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)',
          },
        ]}
      >
        <View style={getStyles(activeColors).totalRow}>
          <Text style={[getStyles(activeColors).totalLabel, { color: activeColors.subtext }]}>
            Tổng cộng dự tính:
          </Text>
          <Text style={[getStyles(activeColors).totalValue, { color: activeColors.text }]}>
...
const getStyles = (colors) =>
  StyleSheet.create({
    ...
    rejectedText: { color: colors.subtext, textDecorationLine: 'line-through' }, 
    itemPrice: { color: colors.primary, fontSize: 14, fontWeight: '600' }, 
    urgentBadge: {
    ...
    urgentText: { color: colors.secondary, fontSize: 8, fontWeight: '900' }, 
    actionRow: {
      flexDirection: 'row',
