      ...
      >
        ...
        <View style={[getStyles(theme).content, { backgroundColor: theme.colors.background }]}>
          <View style={getStyles(theme).titleRow}>
            <View style={{ flex: 1 }}>
              <Text style={[getStyles(theme).name, { color: theme.colors.text }]}>
                {motor?.name || 'Motorcycle'}
              </Text>
              <Text style={[getStyles(theme).category, { color: theme.colors.subtext }]}>
                {motor?.brand} • High Performance
              </Text>
            </View>
            <Text style={[getStyles(theme).price, { color: theme.colors.primary }]}>
              {motor?.price}
            </Text>
          </View>
          ...
          {!isOwned && motor?.colors && (
            <View style={getStyles(theme).section}>
              <Text style={[getStyles(theme).sectionTitle, { color: theme.colors.text }]}>
                Màu sắc
              </Text>
              <View style={getStyles(theme).colorGrid}>
                {motor.colors.map((c) => (
                  ...
                  >
                    ...
                    <Text
                      style={[
                        getStyles(theme).colorLabel,
                        { color: theme.colors.subtext },
                        selectedColor === c.id && getStyles(theme).activeLabel,
                      ]}
                    >
                      {c.label}
                    </Text>
          ...
          {activeTab === 'overview' && (
            <Animated.View entering={FadeInDown} style={getStyles(theme).tabContent}>
              <Text style={[getStyles(theme).description, { color: theme.colors.subtext }]}>
                {motor?.description}
              </Text>
              <GlassCard
                style={[
                  getStyles(theme).featureCard,
              ...
              >
                <View style={getStyles(theme).featureItem}>
                  ...
                  <View style={getStyles(theme).fInfo}>
                    <Text style={[getStyles(theme).fTitle, { color: theme.colors.text }]}>
                      Công nghệ Sugomi
                    </Text>
                    <Text style={[getStyles(theme).fDesc, { color: theme.colors.subtext }]}>
                      Thiết kế tối ưu khí động học và sức mạnh.
                    </Text>
                  </View>
                </View>
                <View style={getStyles(theme).featureItem}>
                  ...
                  <View style={getStyles(theme).fInfo}>
                    ...
                    <Text style={[getStyles(theme).fDesc, { color: theme.colors.subtext }]}>
                      Chính sách bảo hành toàn diện từ AnhEmMotor.
                    </Text>
                  </View>
                </View>
              </GlassCard>
          ...
          {activeTab === 'specs' && (
            <Animated.View entering={FadeInDown} style={getStyles(theme).specGrid}>
              {motor?.specs?.map((s, i) => (
                <View
                  key={i}
                  style={[getStyles(theme).specItem, { backgroundColor: theme.colors.card + '33' }]}
                >
                  <Text style={[getStyles(theme).specVal, { color: theme.colors.text }]}>
                    {s.value}
                  </Text>
                  <Text style={[getStyles(theme).specLabel, { color: theme.colors.subtext }]}>
                    {s.label}
                  </Text>
                </View>
              ))}
